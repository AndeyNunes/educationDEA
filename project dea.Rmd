...
data <- read_csv("OECDdata.csv")
# please leave this as "read_csv" because "read.csv" loads most of the data
# factors instead of numeric values

summary(data)
```

We can see that there are three observations missing from several of the population education level fields and three zero values for `Total.Spending`. These may need to be excluded from the analysis.


```{r data cleaning}
# remove 0 spending countries and fix the spending %GDP column
data <- data %>%
   filter(`Total Spending` > 0) %>%
   mutate(`Spending as percentage of GDP` = `Total Spending`/GDP/100)
```
...
row.names(x1) <- data$Countries #%>%
   
row.names(y) <- data$Countries
...
x <- data %>% select(Spending.as.percentage.of.GDP)
y <- data %>% select(AveragePISA,Tertiary.Graduation.Rate) ## output
row.names(x) <- data[,1]
row.names(y) <- data[,1]

resmult <- DeaMultiplierModel(x, y, rts = "crs", orientation = "output")
...
##DEA with single output, multiple output, dropping rows with no input values
```{r runmultipledea, dropping rows with no input values}

new_data <- data[-c(4,7,12,19),] #rows with no input values dropped
nx <- new_data %>% select(Spending.as.percentage.of.GDP)
ny <- new_data %>% select(AveragePISA,Tertiary.Graduation.Rate) ## output
row.names(nx) <- new_data[,1]
row.names(ny) <- new_data[,1]

new_resmult <- DeaMultiplierModel(nx, ny, rts = "crs", orientation = "output")

new_multEfficiencyTable <- tibble(
   country = as.character(dimnames(new_resmult$Efficiency)[[1]]),
   efficiency = as.numeric(new_resmult$Efficiency)) %>%
   arrange(desc(efficiency))
```


```{r inspect resmult, new_resmult}
pander(head(multEfficiencyTable, 10), caption = "DEA Output Efficency for Multiple Scores")

pander(head(new_multEfficiencyTable, 10), caption = "DEA Output Efficency for Multiple Scores, rows with no input values dropped")
```

```{r weights for resmult}
pander(head(resmult$uy), caption = "DEA Output Weights for Multiple Scores")

pander(head(resmult$Lambda), caption = "DEA Lambda values for Multiple Scores, rows with no input values dropped")
```

```{r weights for new_resmult}
pander(head(new_resmult$uy), caption = "DEA Output Weights for Multiple Scores")

pander(head(new_resmult$Lambda), caption = "DEA Lambda values for Multiple Scores, rows with no input values dropped")
```
...
dea.plot(x = x, y = y, txt = dimnames(x)[[1]], GRID = T,
         xlab ="Spending as percentage of GDP", ylab = "M",
         ylim = c(1200,1600),
